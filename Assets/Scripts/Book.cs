...
public class Book : MonoBehaviour {
    ...
    public bool pageDragEnd = true;
    public int targetPage = -1;
    private GameObject AKeyObejct;
    private GameObject DKeyObejct;
    //current flip mode
    FlipMode mode;
    ...
    void Update()
    {
        ...
        if (PlayerAttributes.Instance._activingItem == null) return;
        if (PlayerAttributes.Instance._activingItem.GetComponent<Book>() == null && PlayerAttributes.Instance._activingItem.GetComponent<BookItem>() == null) return;
        if (_mouseScrollY > 0)
        {
        ...
        if (pageDragEnd)
        {
            if (!AKeyObejct && currentPage > 0) AKeyObejct = Instantiate(AKey, transform);
            if (!DKeyObejct && currentPage < TotalPageCount) DKeyObejct = Instantiate(DKey, transform);
        }
        else
        {
            Destroy(AKeyObejct);
    ...
    private void ApplyBookPage()
    {
        if (normalBook) return;
        if (currentPage < 6 || currentPage.Equals(30)) return;
        SettingPage();
    }

    public void SettingPage()
    {
        List<List<string>> pageContainer = SubtitleManagement.Instance.PageContent;
        for (int i = 0, len = pageContainer.Count; i < len; ++i)
        {
            // pages
            TMP_Text pageText = pageObjects[i].GetComponent<TMP_Text>();
            pageText.SetText("");
            string sentences = "";
            for (int j = 0, len2 = pageContainer[i].Count; j < len2; ++j)
            {
                // sentences
                sentences += pageContainer[i][j];
                sentences += "\n";
            }
            pageText.SetText(sentences);
        }
    }
    ...
    void Flip()
    {
        ...
        if (!targetPage.Equals(-1)) currentPage = targetPage;
        LeftNext.transform.SetParent(BookPanel.transform, true);
        Left.transform.SetParent(BookPanel.transform, true);
        LeftNext.transform.SetParent(BookPanel.transform, true);
        Left.gameObject.SetActive(false);
    ...
    public void ControlCameraAndCanvas(int plusPage)
    {
        if (_cameras.Length > 0 && _canvases.Length > 0)
        {
            if (plusPage.Equals(2))
            {
                // 右翻
                if (!currentPage.Equals(TotalPageCount - 2))
                {
                    _cameras[currentPage + 1].SetActive(true);
                    _cameras[currentPage].SetActive(true);
                    _canvases[currentPage + 1].SetActive(true);
                    _canvases[currentPage].SetActive(true);
                }

                if (!currentPage.Equals(0))
                {
                    _cameras[currentPage - 2].SetActive(false);
                    _cameras[currentPage - 1].SetActive(false);
                    _canvases[currentPage - 2].SetActive(false);
                    _canvases[currentPage - 1].SetActive(false);
                }
            }
            else if (plusPage.Equals(-2))
            {
                // 左翻
                if (currentPage >= 4)
                {
                    _cameras[currentPage - 3].SetActive(true);
                    _cameras[currentPage - 4].SetActive(true);
                    _canvases[currentPage - 3].SetActive(true);
                    _canvases[currentPage - 4].SetActive(true);
                }
                if (!currentPage.Equals(TotalPageCount))
                {
                    _cameras[currentPage - 1].SetActive(false);
                    _cameras[currentPage - 2].SetActive(false);
                    _canvases[currentPage - 1].SetActive(false);
